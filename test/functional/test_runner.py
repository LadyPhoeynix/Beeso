...
def main():
    ...
    tmpdir = "%s/beeso_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and beesod must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "beesod"]) is not None:
    ...
    #Set env vars
    if "BEESOD" not in os.environ:
        os.environ["BEESOD"] = build_dir + '/src/beesod' + exeext
        os.environ["BEESOCLI"] = build_dir + '/src/beeso-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `beeso-cli help` (`rpc_interface.txt`).
